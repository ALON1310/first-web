...
function StoreScreen({ user, cart, onAddToCart, onShowCart, onLogout }) {
  const jets = useMemo(() => [
    ...
    <div className="store-screen-page">
      <nav className="navbar">
        ...
        <div className="nav-links">
          <button onClick={onShowCart}>Cart ({cart.length})</button>
      ...
      <section className="products-grid">
        {filteredJets.map(jet => (
          <ProductCard key={jet.id} jet={jet} onAddToCart={onAddToCart} />
        ))}
      </section>
    </div>
  );
